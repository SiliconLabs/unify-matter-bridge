...
TEST (TestClusterEmulator, TestClusterEmulatorEmulationClusterRevisionAndFeatureMap)
{
    ...
    EXPECT_EQ(matter_onoff_cluster.clusterId ,chip::app::Clusters::OnOff::Id);
    ...
    for (uint8_t i = 0; i < matter_onoff_cluster.attributeCount; i++)
    {
        if (matter_onoff_cluster.attributes[i].attributeId == emulated_attribute_feature_map)
        {
            ...
            EXPECT_TRUE( emulator.is_attribute_emulated(aPath));
    ...
    EXPECT_TRUE( feature_map_found);
    EXPECT_TRUE( cluster_revision_found);
    ...
    EXPECT_NE( emulator.read_attribute(attribute_id_path, encoder),CHIP_NO_ERROR);
    ...
    // The encoding fails everytime properly due to bad setup. This is a bug in the test code.
    EXPECT_NE( emulator.read_attribute(revision_attribute_id_path, revision_encoder) ,CHIP_NO_ERROR);
}

TEST (TestClusterEmulator, TestClusterEmulatorIdentify)
{
    sl_log_debug(TEST_LOG_TAG, "TestClusterEmulatorEmulation");
    // Initialize the cluster interactor
    unify::matter_bridge::device_translator matter_device_translator(false);
    ...
    EXPECT_EQ( identify_cluster.clusterId ,chip::app::Clusters::Identify::Id);
    ...
    EXPECT_TRUE(identify_type_found);
    ...
    // EXPECT_EQ( emulator.read_attribute(attribute_id_path, encoder) != CHIP_NO_ERROR);
    ...
    EXPECT_EQ( emulator.read_attribute(invalid_attribute_id_path, encoder) ,CHIP_ERROR_INVALID_ARGUMENT);
}
}
}
}
